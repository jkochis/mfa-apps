			...
			if (current == index) {
				...
				for (xmlAttrPtr attr = child->properties; attr != NULL; attr = attr->next) {
					if (xmlStrEqual(attr->name, (xmlChar*)"id")) {
						...
						return [StopFactory stopForStopNode:refStopNode];
					}
				}
			} else {
				current++;
...
- (NSArray *)getAllFiles
{
	...
	NSString *portraitImage = [self getHeaderPortraitImage];
	NSString *landscapeImage = [self getHeaderLandscapeImage];
	if (portraitImage) {
		[array addObject:portraitImage];
	}
	if (landscapeImage) {
		[array addObject:landscapeImage];
	}
	return array;
}
